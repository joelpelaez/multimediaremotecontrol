...
/**
 * It's a subclass of {@link org.darksoft.android.multimedia.remotecontrol.ConnectClient ConnectClient}
 * for manage Bluetooth connections to Remote Control Server.
 * 
...
public class BluetoothClient extends ConnectClient {
	...
	//final public UUID mUUIDService = UUID.fromString("3d84ccaa-c215-43a2-8511-0e7cbb40f96c");
	final public UUID mUUIDService = UUID.fromString("00001101-0000-1000-8000-00805F9B34FB");
    ...
    /**
     * Start a connection a Bluetooth device using a specific MAC Address.
     * This method execute {@link #startConnection(String)} in another thread.
     * 
     * @param address A {@link java.lang.String String} with the MAC Address
     * @param activity Actually {@link android.app.Activity Activity}, used for invoke #{@link android.app.AlertDialog AlertDialogs}.
     */
	...
	/**
	 * Convert a {@link java.lang.String String} to a byte array for send on a socket.
	 * 
	 * @param str A simple {@link java.lang.String String} object.
	...
	/**
	 * Start a connection a Bluetooth device using a specific MAC Address.
	 * This method is only for use in this class and subclases.
	 * For direct call use {@link #connectToDevice(String, Activity)}
	 * 
	 * @param address A {@link java.lang.String String} with MAC Address of Bluetooth's Server Device.
	 */
	protected void startConnection(String address) {
		...
		mRootActivity.runOnUiThread(new Runnable() {
			public void run() {
				String title = mRootActivity.getResources().getString(R.string.loading_title);
				String message = mRootActivity.getResources().getString(R.string.loading_message);
				progress = ProgressDialog.show(mRootActivity, title, message, true);
			}
		});
		try {
			mSocket = mClient.createInsecureRfcommSocketToServiceRecord(mUUIDService);
		} catch (IOException e) {
			mRootActivity.runOnUiThread(new Runnable() {
				public void run() {
					progress.cancel();
	...
	/**
	 * Get a {@link java.util.Set Set} object with {@link android.bluetooth.BluetoothDevice BluetoothDevices}
	 * objects with information of the current paired Bluetooth devices.
	 * 
	 * @return A {@link java.util.Set Set} of paired {@link android.bluetooth.BluetooothDevice BluetoothDevice} objects.
	 */
	static public Set<BluetoothDevice> getDeviceList() {
		BluetoothAdapter bluetooothAdapter = BluetoothAdapter.getDefaultAdapter();
		Set<BluetoothDevice> devices = bluetooothAdapter.getBondedDevices();
		return devices;
	}
	
	/**
	 * Start a discovery of Bluetooth devices. Use the {@link android.bluetooth.BluetoothAdapter#startDiscovery() startDiscovery()}
	 * method, so necessary use a {@link android.}BroadcastReceiver to get a list of the discovered Bluetooth devices.
	 */
	...
	/**
	 * Cancel a discovery of devices. This method check if a discovery is running. If true will
	 * cancel the discovery, else will do nothing.
	 */
